...
export class visionaiController implements Controller {
    ...
    private initializeRoutes() {
        this.router.post('/visionai', this.GpCreate);
this.router.get('/visionai/userid/created_by', this.GpGetNounCreatedBy);
        //#@gepdelimeterone@#
        //#@ssofacebookapiroute@#
        //#@ssogithubapiroute@#
        //#@gepbankingapiroute@#
...
public GpCreate(req: Request, res: Response) {
        ...
        new ApiAdapter().post(Constant.VISIONAIURL + `${req.url}` , req.body)
...
public GpGetNounCreatedBy(req: Request, res: Response) {
        ...
        new ApiAdapter().get(Constant.VISIONAIURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
